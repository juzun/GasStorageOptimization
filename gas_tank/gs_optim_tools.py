...
def collect(solved_storages) -> tuple:
    ...
    dates = [date_min + dt.timedelta(days=i) for i in range(0,(date_max-date_min).days+1)]
    ...
    total_data = pd.DataFrame(
        list(zip(
            list(total_operations.values()),list(total_gs_state.values()),
            list(total_max_operations.values()),list(total_wgv.values()))),
    ...
    total_daily_export = pd.DataFrame(
        total_data[['Rok','M','W/I','Stav', 'Stav %','Max C', 'WGV']],
        index=dates)
    ...
    total_monthly_export['Stav %'] = total_monthly_export['Stav']/total_monthly_export['WGV']

    fig = po.Figure()
    fig.add_trace(po.Scatter(x=dates, y=list(total_max_operations.values()), name='Max. operations', line_color='#ffa600', mode='lines'))
    fig.add_trace(po.Scatter(x=dates, y=list(total_operations.values()), name='Operations', fill='tozeroy', line_color='#74d576', mode='lines'))
    fig.add_trace(po.Scatter(x=dates, y=list(total_gs_state.values()), name='GS state', fill='tozeroy', line_color='#34dbeb', yaxis = 'y2'))
    fig.update_layout(
    ...
    total_daily_export = total_daily_export[
        ['Rok','M','W/I',*[f'W/I {self.name}' for self in solved_storages],'Stav','Stav %',*[f'Stav {self.name}' for self in solved_storages],
            'Max C',*[f'Max C {self.name}' for self in solved_storages]]
    ]
